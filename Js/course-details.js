...
async function fetchCourseDetails() {
  try {
    ...
    // optional: update header dynamically
    ...
    if (subtitleEl) {
      subtitleEl.textContent = `${course.courseName} • Instructor: ${course.instructor || 'N/A'}`;
    }
...
function renderAssessments(assessments, course) {
  ...
  assessments.forEach(a => {
    const statusBadge =
      a.status === 'completed' ? 'text-bg-success' : 'text-bg-warning';
...
function attachRowListeners() {
  ...
  tbody.querySelectorAll('.edit-btn').forEach(btn => {
    btn.addEventListener('click', async e => {
      ...
      const status = Number(earnedMarks) === Number(totalMarks) ? 'completed' : 'pending';
...
function updateSummary(assessments) {
  ...
  const totalEarned = assessments.reduce(
    (sum, a) => sum + (parseFloat(a.earnedMarks) || 0),
    0
  );

  const totalMarks = assessments.reduce(
    (sum, a) => sum + (parseFloat(a.totalMarks) || 0),
    0
  );
